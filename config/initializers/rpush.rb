Rpush.configure do |config|
  ...
  # The maximum number of notifications to load from the store every
  # `push_poll` seconds.
  # If some notifications are still enqueued internally,
  # Rpush will load the batch_size less
  # the number enqueued. An exception to this is if the service is
  # able to receive multiple
  ...
  config.log_level = if defined?(Rails) && Rails.logger
                       Rails.logger.level
                     else
...
Rpush.reflect do |on|
  ...
  # on.notification_id_failed do |app, notification_id, error_code,
  # error_description|
  # end
